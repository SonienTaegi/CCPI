...
int main(void) {
	...
	while(mContext.isValid) {
		...
		if(pBuffer) {
			...
			memcpy(pV, pSrc, mContext.nSizeV);	pV += mContext.nSizeV;
			pBufferRenderCurrent->nFilledLen += pBuffer->nFilledLen;
			...
			OMXsonienBufferPut(mContext.pManagerCamera, pBuffer);
		}
		else {
			usleep(100);
		}
